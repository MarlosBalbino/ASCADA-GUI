...
class SerialStream:
    # TODO: This code is the implementation of Serial Stream. Needs to be changed to implement a USB Streaming.
    ...
    def __init__(self, port, out_queue, in_ts_queue, in_flag_queue):
        ...
        :param out_queue: Receives data from DataGateway to sent through serial. Its elements are
         tuples in the format: (id, type, value). Where:
            id: An integer in the interval: 1:13 (including);
            type: Ane of the intens of DataTypes;
            value: A number that can be represented in the specified type
        :param in_ts_queue: Receives TimeSeries samples from serial to share through DataGateway.
         Its elements are tuples in the format: (id, time, value). Where:
            id: An integer in the interval: 1:14 (including);
            time: time sample (Number);
            value: value sample (Number);
        :param in_flag_queue: Receives data from serial to share through DataGateway. Its elements
